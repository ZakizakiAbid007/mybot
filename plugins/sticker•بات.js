//código creado x The Carlos 👑 
import Jimp from "jimp";
import { sticker } from '../lib/sticker.js';
import fs from 'fs';
import path from 'path';

let handler = async (m, { conn, text }) => {
  
  // 1. التحقق من الإدخال
  if (!text) return m.reply("⚠️ اكتب شيئًا بعد .بات\nمثال: *.بات مرحبا يا صديقي 🤣*");

  // 2. تقييد طول النص بـ 20 كلمة
  let words = text.split(/\s+/).slice(0, 20);
  text = words.join(' ');

  // 3. اختيار صورة خلفية عشوائية (الميزة الجديدة)
  let randomNum = Math.floor(Math.random() * 4) + 1; // يختار رقماً عشوائياً بين 1 و 4
  let imagePath = `./src/sticker/pat${randomNum}.jpg`; // يتم تحميل pat1.jpg, pat2.jpg, إلخ
  let image = await Jimp.read(imagePath);

  // 4. إعدادات الخط (أبيض) والقيود
  let fontSize = 200;
  let font = await Jimp.loadFont(Jimp.FONT_SANS_128_WHITE); // استخدام خط أبيض
  const maxWidth = image.bitmap.width - 60;
  const maxHeight = 500;

  // 5. تعديل حجم الخط ديناميكياً ليناسب المساحة المحددة
  while (Jimp.measureTextHeight(font, text, maxWidth) > maxHeight && fontSize > 64) {
    fontSize -= 16;
    font = await Jimp.loadFont(
      fontSize > 128 ? Jimp.FONT_SANS_128_WHITE :
      fontSize > 64 ? Jimp.FONT_SANS_64_WHITE :
      Jimp.FONT_SANS_32_WHITE
    );
  }

  // 6. حساب موضع النص العمودي (بالقرب من الأسفل)
  const textHeight = Jimp.measureTextHeight(font, text, maxWidth);
  const y = image.bitmap.height - textHeight - 30; // يوضع النص بمقدار 30 بكسل من الحافة السفلية

  // 7. تحميل خط الظل (أسود)
  let shadow = await Jimp.loadFont(
    fontSize > 128 ? Jimp.FONT_SANS_128_BLACK :
    fontSize > 64 ? Jimp.FONT_SANS_64_BLACK :
    Jimp.FONT_SANS_32_BLACK
  );

  // 8. تحديد إزاحات الظل (4 اتجاهات)
  let offsets = [
    { x: -3, y: 0 }, { x: 3, y: 0 },
    { x: 0, y: -3 }, { x: 0, y: 3 }
  ];

  // 9. طباعة الظل (أسود) أولاً (الميزة الجديدة)
  offsets.forEach(o => {
    image.print(
      shadow,
      30 + o.x, // إزاحة أفقية
      y + o.y, // إزاحة عمودية
      {
        text: text,
        alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER,
        alignmentY: Jimp.VERTICAL_ALIGN_TOP
      },
      maxWidth,
      textHeight
    );
  });

  // 10. طباعة النص الأساسي (أبيض) فوق الظل
  image.print(
    font,
    30, // بدون إزاحة
    y, // بدون إزاحة
    {
      text: text,
      alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER,
      alignmentY: Jimp.VERTICAL_ALIGN_TOP
    },
    maxWidth,
    textHeight
  );

  // 11. تحويل الصورة الناتجة إلى ملصق وإرسالها
  let buffer = await image.getBufferAsync(Jimp.MIME_PNG);
  let stiker = await sticker(buffer, false, '𝕭𝖑𝖆𝖈𝖐 𝕮𝖑𝖔𝖛𝖊𝖗', 'The Carlos 👑');

  if (!stiker) return m.reply("❌ لم نتمكن من توليد الملصق.");
  
  // ... (منطق contextInfo للإرسال)
  const imgFolder = path.join('./src/img');
  const imgFiles = fs.readdirSync(imgFolder).filter(f => /\.(jpe?g|png|webp)$/i.test(f));
  let contextInfo = {};
  if (imgFiles.length > 0) {
    contextInfo = {
      externalAdReply: {
        title: '𝕭𝖑𝖆𝖈𝖐 𝕮𝖑𝖔𝖛𝖊𝖗 | باتريك 👻',
        body: 'Dev • The Carlos 👑',
        mediaType: 2,
        thumbnail: fs.readFileSync(path.join(imgFolder, imgFiles[0]))
      }
    };
  }

  await conn.sendMessage(m.chat, { sticker: stiker, contextInfo }, { quoted: m });
};

handler.help = ['pat'];
handler.tags = ['sticker'];
handler.command = ['pat', 'بات'];
handler.group = false;
handler.register = true;

export default handler;